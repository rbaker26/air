...
url = 'https://www.airnow.gov/index.cfm?action=airnow.local_city&zipcode={zipcode}&submit=Go'
...
if __name__ == '__main__':
    air = build_dict(url.format(zipcode=zipcode))
    import json

    # templateized so that the year and semester can easily be changed with globals
    fileName = 'air-{zipcode}.json'
    with open(cFileName.format(zipcode=zipcode), 'w') as file:
        json.dump(air, file, sort_keys=True, indent=4)
